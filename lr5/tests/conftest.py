...
def existing_user(db_connector, existing_role_user):
    user_data = (1, 'admin', 'qwerty', 'Студент', 'Студент', int(existing_role_user.id))
...
def example_users(db_connector, example_roles):
    data = [
        (1, 'admin', 'qwerty', 'admin', 'admin', example_roles[0].id), 
        (2, 'user', 'qwerty', 'user', 'user', example_roles[1].id)
    ]
    row_class = namedtuple('Row', ['id', 'login', 'password', 'first_name', 'last_name', 'role_id'])
