...
def edit(user_id):
    ...
    if request.method == 'POST':
        ...
        if current_user.role == 'admin':
            ...
            user_data['role_id'] = role_repository.get_with_name(current_user.role).id
...
def visitor_log():
    if current_user.role != 'admin':    
        logs_count = journal_repository.get_total_count_with_user_id(current_user.id)
    else:
    ...
    if page > (logs_count // per_page) + 1:
        page = 1
    if current_user.role != 'admin':
        logs = journal_repository.get_page_with_user_id(current_user.id, per_page, page)
    else:
